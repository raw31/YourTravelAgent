"""v4 — v3's conversation, plus accepting a typed-out booking as a fresh
submission (not just a link or a screenshot).

Starts as an exact copy of v3.py -- v3.py itself is untouched and keeps
running unaffected; this is a separate module with its own session
store, same as v1/v2/v3 each having their own.
    ...
    for m in items:
        if m.get("type") in ("image", "document") and m.get("media_id"):
            ...
            if dl:
                ...
                print(f"[wa v4] downloaded media: {len(data)} bytes, {mime}", flush=True)
...
def _present_deal(frm: str, packet) -> None:
    ...
    print(f"[wa v4] batch for {frm} complete (matched={matched} bookable={bookable})", flush=True)


def _run_extraction(frm: str, url, media, page_text: str | None = None) -> None:
    from yta import whatsapp
    from yta.pipeline import extract

    wa_send(frm, random.choice(_CHECKING_PHRASES))
    print(f"[wa v4] extracting: url={url!r} has_media={bool(media)} has_text={bool(page_text)}", flush=True)
    packet = extract(url or "", render=bool(url), media=media, page_text=page_text, log_sink=[])
    print(f"[wa v4] extraction done: hotel={packet.hotel.name!r} status={packet.status}", flush=True)
...
def _handle_awaiting_field(frm: str, session: dict, items: list) -> None:
    ...
    print(f"[wa v4] clarification filled: {list(fields.keys())}; note={clarify!r}", flush=True)

    if not fields and not clarify:
        attempts = session.get("unproductive_attempts", 0) + 1
        if attempts >= _MAX_UNPRODUCTIVE_ATTEMPTS:
            print(f"[wa v4] {frm} gave up after {attempts} unproductive replies", flush=True)
            with _WA_SESSIONS_LOCK:
    ...
    print(f"[wa v4] still missing after clarification: {still_missing}", flush=True)
    if still_missing:
        with _WA_SESSIONS_LOCK:
...
def _handle_presented(frm: str, session: dict, items: list, button_id) -> None:
    ...
    if is_confirm:
        ...
        print(f"[wa v4] {frm} confirmed, lead {ref} (referred_by={referred_by})", flush=True)
        return
    ...
    if is_decline:
        ...
        print(f"[wa v4] {frm} declined, lead {ref} (referred_by={referred_by})", flush=True)
        return
...
def handle_batch(frm: str, items: list) -> None:
    ...
    if session is not None and time.time() - session.get("last_activity", 0) > _SESSION_MAX_AGE_SEC:
        print(f"[wa v4] session for {frm} abandoned (idle > {_SESSION_MAX_AGE_SEC}s) — clearing it "
    ...
    try:
        ...
        if referral_match:
            ...
            print(f"[wa v4] {frm} mentioned referral {referral_match.group(0).upper()}", flush=True)
        ...
        if url:
            ...
            if session is not None:
                print(f"[wa v4] {frm} sent a new link mid-conversation — dropping the old session", flush=True)
                with _WA_SESSIONS_LOCK:
                    _WA_SESSIONS.pop(frm, None)
        ...
        if session is None:
            ...
            if _looks_like_a_query(text):
                _run_extraction(frm, None, None, page_text=text)
                return
            wa_send(frm, _ONBOARDING_TEXT)
            return
        ...
        print(f"[wa v4] ERROR handling batch: {type(e).__name__}: {e}", flush=True)
        with _WA_SESSIONS_LOCK:
            _WA_SESSIONS.pop(frm, None)
