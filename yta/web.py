...
# webhook front door and a text-formatted reply. The actual conversation
# logic (session state, what counts as an answer vs a new request, what
# to say when) lives in yta/wa_flows/*—pluggable per YTA_WA_FLOW so
# more than one flow design can exist side by side. web.py only owns the
# HTTP surface and the per-sender debounce buffer below.
...
def _process_batch(frm: str) -> None:
    ...
    print(f"[wa] processing batch for {frm}: {len(items)} message(s), "
          f"flow={wa_flows.active_flow_name()!r}", flush=True)
    wa_flows.handle_batch(frm, items)
                ...
                if not client.configured():
                    ...
                    if det.options:
                        ...
                        if packet.requested_offer.room_name:
                            ...
                            total_opts = sum(len(g.options) for g in rgr.groups)
                            packet.log(
                                f"no requested room name — listed {len(rgr.groups)} "
                                f"room(s), {total_opts} option(s) total")
            except TripJackError as e:
